              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Show special indicator when this is the final answer -->
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const subQSection = text.match(/### (?:分析的子问题|Sub-questions for analysis)\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
    ...
    const relSection = text.match(/### (?:【关系链】|【Relationship Chain】)\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const reasonMatch = text.match(/### (?:采访对象选择理由|Interviewee selection rationale)\n([\s\S]*?)(?=\n---\n|\n### (?:采访实录|Interview transcripts))/)
    if (reasonMatch) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        // Split questions by number prefix
        const questions = qText.split(/\n\d+\.\s+/).filter(q => q.trim())
        if (questions.length > 0) {
          // Handle special case where first question starts with "1."
      ...
      if (quotesMatch) {
        ...
        // Fallback: match > "text" or > curly/CJK quotes
        if (!quoteMatches) {
...
// Interview Display Component (Panorama ends here) - Conversation Style (Q&A Format)
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(No response from this platform)' || t === '（该平台未获得回复）' || t === '(该平台未获得回复)' || t === '[无回复]' || t === '[No response]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // If no numbering found or only one, return as-is
      if (matches.length <= 1) {
...
const renderMarkdown = (content) => {
  ...
  // Strip leading h2 (## xxx), since section title is already shown in the outer wrapper
  ...
  // Clean up <br> between block element closing and paragraph opening tags
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-contiguous ordered list numbering: keep incrementing when single-item <ol> tags are separated by paragraph content
...
const extractFinalContent = (response) => {
  ...
  // If starts with ## or # or >, likely raw markdown content
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // If content is long and contains markdown formatting, try stripping the thinking process
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove leading "Thought:" reasoning section
