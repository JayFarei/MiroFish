        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 "Simulation" and Step 5 "Deep Interaction" must be launched live and do not support historical playback</span>
            </div>
          </div>
        </div>
      </Transition>
...
let pendingState = null  // Target state pending execution
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    const x = offset * 35
    // Adjust start position, close to title with adequate spacing
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update pending target state (always record latest regardless of animation status)
        ...
        // No processing needed if target state matches current
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Use debounced delay for state transitions to prevent flickering
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Brief delay before executing to avoid rapid toggling
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin, shrink viewport bottom so more scroll is needed to trigger expand
...
// Reload data on component activation (for keep-alive)
onActivated(() => {
  loadHistory()
})
...
.grid-pattern {
  ...
  /* Anchor at top-left, height changes only extend bottom without shifting grid */
