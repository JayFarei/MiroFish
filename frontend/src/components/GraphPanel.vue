...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transitioned from simulating to non-simulating, show finish hint
...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    // Calculate curvature: spread apart for multiple edges, straight for single edge
    let curvature = 0
    if (totalCount > 1) {
      ...
      // Flip curvature if edge direction is reversed from normalized
      // Ensures all edges distribute in the same reference frame without overlapping
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const getLinkPath = (d) => {
    ...
    // More edges result in a larger offset ratio
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        ...
        if (!d._isDragging && distance > 3) {
          // Restart simulation only on first real drag detection
          d._isDragging = true
