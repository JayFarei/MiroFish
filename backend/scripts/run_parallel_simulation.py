...
- After simulation completes, enters command wait mode instead of closing immediately
...
if sys.platform == 'win32':
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        Fixes third-party libraries (like OASIS) that read files without specifying encoding.
        """
...
class ParallelIPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        """
        # If platform specified, interview on that platform only
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        # Split both_platforms_interviews into both platform lists
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows actions.jsonl to show real entity names instead of "Agent_0" placeholders
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Last processed rowid value (using rowid instead of created_at due to format differences across platforms)
        agent_names: agent_id -> agent_name mapping

    Returns:
        (actions_list, new_last_rowid)
        - actions_list: Action list, each element contains agent_id, agent_name, action_type, action_args (with context)
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Supports dual LLM config for faster parallel simulation:
    ...
    # Fall back to config if .env has no model name
    if not llm_model:
...
async def run_twitter_simulation(
    ...
    # Note: Do not close environment, keep it available for Interview use
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation scenario: after simulation completes, wait for interview commands.
