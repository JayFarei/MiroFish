...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # - running: simulation running, preparation was long complete
        # - completed: simulation complete, preparation was long complete
        # - stopped: simulation stopped, preparation was long complete
        # - failed: simulation failed (but preparation is complete)
...
def prepare_simulation():
    ...
    4. Intelligently generate simulation config with LLM (with retry)
    ...
    Request (JSON):
        {
            ...
            "use_llm_for_profiles": true,                 // Optional, use LLM for persona generation
            "parallel_profile_count": 5,                  // Optional, parallel persona count, default 5
    ...
    try:
        ...
        # This allows the frontend to immediately get the expected Agent total after calling prepare
        try:
            ...
            filtered_preview = reader.filter_defined_entities(
                ...
                enrich_with_edges=False  # Don't fetch edge info, faster
            )
            ...
            # Failure doesn't block subsequent flow; background task will retry
...
def get_prepare_status():
    ...
    Request (JSON):
        {
            ...
            "simulation_id": "sim_xxxx"      // Optional, simulation ID (for checking completed prep)
        }
    ...
    try:
        ...
        # If simulation_id provided, check if preparation is already complete first
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
        ...
        if not task:
            # Task doesn't exist, but if simulation_id is available, check if already prepared
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def get_simulation_history():
    ...
    Used for homepage history display, returns simulation list enriched with project name, description, etc.
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get run state (reads user-set actual rounds from run_state.json)
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time viewing during generation
    - Returns extra metadata (e.g. file modification time, whether generation is in progress)
    - Can return partial info even if config has not finished generating
...
def get_simulation_config(simulation_id: str):
    ...
    Returns:
        ...
        - generation_reasoning: LLM reasoning explanation for the config
    """
    try:
        manager = SimulationManager()
...
def start_simulation():
    ...
    About the force parameter:
        - When enabled, if simulation is running or completed, it first stops and clears run logs
        ...
        - Useful when needing to rerun a simulation
    ...
    About enable_graph_memory_update:
        ...
        - This allows the graph to "remember" the simulation, for later analysis or AI chat
    ...
    try:
        ...
        # Check if simulation is prepared and ready
        manager = SimulationManager()
        state = manager.get_simulation(simulation_id)
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                # Preparation done, check if there is a running process
...
def get_run_status_detail(simulation_id: str):
    ...
    Used for frontend to display real-time activity feed
...
def get_agent_stats(simulation_id: str):
    ...
    Used for frontend to display agent activity rankings, action distribution, etc.
    """
    try:
...
def interview_agent():
    ...
    Note: This requires the simulation environment to be in running state
    (after completing simulation loop, entering command wait mode)
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment not running or closed. Please ensure simulation is complete and in command wait mode."
            }), 400
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What do you think about B?"  // no platform uses the default
                }
            ],
...
def close_simulation_env():
    ...
    Sends a close command to the simulation, causing it to gracefully exit command wait mode.
