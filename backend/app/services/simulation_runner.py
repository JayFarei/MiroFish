...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # max simulation rounds (optional, to cap long simulations)
        ...
        Args:
            ...
            max_rounds: max simulation rounds (optional, to cap long simulations)
        ...
        try:
            ...
            # if max_rounds specified, add to command args
            if max_rounds is not None and max_rounds > 0:
                cmd.extend(["--max-rounds", str(max_rounds)])
            
            # create main log file to avoid blocking from full stdout/stderr pipe buffers
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')
            
            # set subprocess env vars to ensure UTF-8 encoding on Windows
            # fixes third-party libs (e.g. OASIS) opening files without explicit encoding
            env = os.environ.copy()
            ...
            # set cwd to simulation directory (database files are generated there)
            # start_new_session=True creates a new process group, killable via os.killpg
            process = subprocess.Popen(
                ...
                start_new_session=True,  # create new process group, ensures all related processes are killed on server shutdown
            )
            
            # save file handles for later cleanup
            cls._stdout_files[simulation_id] = main_log_file
            cls._stderr_files[simulation_id] = None  # separate stderr file no longer needed
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # update round info (from round_end event)
                                    ...
                                    # overall round = max across both platforms
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # overall time = max across both platforms
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Platform is considered enabled if its actions.jsonl file exists.
    ...
    def _read_actions_from_file(
        ...
        Args:
            ...
            default_platform: default platform (used when record lacks a platform field)
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # get platform: prefer record's platform field, else use default
    ...
    def get_all_actions(
        ...
        # read Twitter action file (platform auto-set to twitter from path)
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        ...
        # read Reddit action file (platform auto-set to reddit from path)
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
        ...
        if not actions:
            ...
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # legacy format should have platform field in records
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        Note: config file (simulation_config.json) and profile files are not deleted.
        ...
        # list of directories to clean (contain action logs)
    ...
    def cleanup_all_simulations(cls):
        ...
        # check if there is anything to clean (avoid noisy logs when nothing running)
        ...
        for simulation_id, process in processes:
            try:
                if process.poll() is None:  # process still running
                    ...
                    # also update state.json to stopped
                    try:
                        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def register_cleanup(cls):
        ...
        # in Flask debug mode, only register in the reloader child process (the one actually running the app)
        ...
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # mark registered so child processes don't retry
            return
        ...
        def cleanup_handler(signum=None, frame=None):
            ...
            if signum == signal.SIGINT and callable(original_sigint):
                ...
                # original handler not callable (e.g. SIG_DFL), use default behavior
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        Returns:
            True if alive, False if shut down
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: target platform (optional)
                ...
                - None: interview both platforms for dual-platform simulations
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"Simulation not running or closed, cannot execute interview: {simulation_id}")
    ...
    def interview_agents_batch(
        ...
        Args:
            simulation_id: simulation ID
            interviews: list of interviews, each with {"agent_id": int, "prompt": str, "platform": str (optional)}
    ...
    def interview_all_agents(
        ...
        Args:
            ...
            platform: target platform (optional)
                ...
                - "reddit": Reddit only
                - None: both platforms per agent in dual-platform simulations
            timeout: timeout in seconds
    ...
    def close_simulation_env(
        ...
        Sends a close-env command to the simulation so it exits the command-waiting mode gracefully.
        ...
        try:
            ...
            # timeout may mean environment is already closing
            return {
                "success": True,
    ...
    def get_interview_history(
        ...
        if platform in ("reddit", "twitter"):
            ...
            # when platform not specified, query both
        ...
        # if multiple platforms queried, cap total results
