...
Generates simulation reports using a ReACT pattern with LangChain + Zep
...
3. Each section uses multi-round ReACT (reasoning + acting) pattern
4. Supports chat with users, autonomously calling retrieval tools
"""
...
class ReportLogger:
    ...
    Generates agent_log.jsonl in the report folder, recording each step in detail.
    ...
    def log_section_content(
        ...
        """Log section content generation done (content only, not full section completion)"""
        self.log(
    ...
    def log_section_full_complete(
        ...
        Frontend should monitor this log to determine when a section is truly done
        """
        self.log(
...
[Returns]
- Related fact originals (can be cited directly)
...
4. Calls the /api/simulation/interview/batch API for real interviews on both platforms
5. Integrates all interview results, providing multi-perspective analysis

[Use Cases]
- Need to understand different role perspectives on events (What do students think? What does media say? What's the official stance?)
...
PLAN_SYSTEM_PROMPT = """\
You are an expert in writing "Future Prediction Reports", with a "God's-eye view" of the simulated world — you can observe every Agent's behavior, statements, and interactions in the simulation.

[Core Concept]
We have built a simulated world and injected specific "simulation requirements" as variables. The evolution results of the simulated world are predictions of what might happen in the future. What you are observing is not "experimental data", but a "rehearsal of the future".
...
The simulated world is a rehearsal of the future. We injected specific conditions (simulation requirements) into the simulated world. Agent behaviors and interactions in the simulation are predictions of future human behavior.
...
2. [Must Cite Agent Original Behaviors]
...
3. [Language Consistency]
   - The report must always be written in English.
   - Tool results may contain content in various languages; translate them into fluent English before writing into the report.
...
[Correct Example]
```
This section analyzes the opinion propagation dynamics of the event. Through in-depth analysis of simulation data, we found...

**Initial Ignition Phase**

As the first scene of public opinion, the platform took on the core function of initial information release:
...
class ReportAgent:
    ...
    2. Generation stage: generate content per section, calling tools as needed
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            ...
            # ========== Backward-compatible old tools (internally redirected) ==========
    ...
    # Valid tool names for fallback bare JSON parsing validation
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only tried when format 1 fails, to avoid false matches in text
    ...
    def _generate_section_react(
        ...
        min_tool_calls = 0  # Min tool call count (0 when using Claude Code headless, tools are simulated in-prompt)
        conflict_retries = 0  # Consecutive conflicts: tool call + Final Answer together
        ...
        for iteration in range(max_iterations):
            ...
            if has_tool_calls and has_final_answer:
                logger.warning(
                    ...
                    f"LLM output both tool call and Final Answer, accepting Final Answer"
                )
                # Strip tool calls and accept the Final Answer directly
            ...
            if has_final_answer:
                # Insufficient tool calls, reject and request more
            ...
            if has_tool_calls:
                # Tool quota exhausted, request Final Answer
            ...
            # Use this content as final answer directly
    ...
    def generate_report(
        ...
        """
        Generate full report (real-time per-section output)
        ...
        try:
            ...
            # Stage 2: Generate sections (save per section)
    ...
    def chat(
        ...
        Agent can autonomously call retrieval tools to answer questions
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Console output logs (INFO, WARNING, etc.) from report generation,
        different from agent_log.jsonl structured logs.
        
        Args:
            report_id: Report ID
            from_line: Line to start reading from (for incremental fetch, 0 = beginning)
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        1. Remove duplicate Markdown heading lines at content start
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Check for duplicate section title (skip within first 5 lines)
                if i < 5:
            ...
            # If previous line was a skipped heading and current line is blank, skip
    ...
    def update_progress(
        ...
        Frontend reads progress.json for real-time progress
        """
        cls._ensure_report_folder(report_id)
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        Assemble from saved section files with heading cleanup
        """
        folder = cls._get_report_folder(report_id)
        ...
        # Post-process: clean up heading issues in full report
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Skip separator line right after heading
                i += 1
                continue
    ...
    def get_report(cls, report_id: str) -> Optional[Report]:
        ...
        if not os.path.exists(path):
            # Backward compatible: check files stored directly in reports dir
