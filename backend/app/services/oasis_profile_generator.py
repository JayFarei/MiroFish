...
Converts entities from the Zep graph into Agent Profile format for the OASIS simulation platform.
...
class OasisProfileGenerator:
    ...
    Converts Zep graph entities into Agent Profiles for OASIS simulation.

    Features:
    1. Uses Zep graph retrieval for richer context
    2. Generates detailed personas (basic info, career, personality, social media behavior, etc.)
    3. Distinguishes individual entities from abstract group entities
    """
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Zep graph hybrid search to get rich info about an entity.

        Zep has no built-in hybrid search API, so edges and nodes are searched
        separately then merged. Uses parallel requests for efficiency.
    ...
    def _generate_profile_with_llm(
        ...
        - Group/institution entity: generates a representative account profile
        """
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        if json_match:
            ...
            # 3. Fix newline chars inside strings
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for opinion simulation, maximally reflecting existing real-world situations. Must return valid JSON format; all string values must not contain unescaped newlines. Write in English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, maximally reflecting existing real-world situations.
...
2. persona: Detailed persona description (2000 words plain text), should include:
   ...
   - Position and views (attitude toward topics, content that might provoke or move them)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for an institutional/group entity, maximally reflecting existing real-world situations.
...
2. persona: Detailed account profile description (2000 words plain text), should include:
   ...
   - Position and attitude (official stance on core topics, approach to handling controversies)
   - Special notes (represented group profile, operational habits)
   - Institutional memory (important part of the institutional persona, introduce this institution's connection to the event and its existing actions and reactions in the event)
...
- Institutional account statements should align with its identity positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            realtime_output_path: File path for real-time writing (writes after each generation)
