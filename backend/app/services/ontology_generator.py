...
API 1: Analyzes text content to generate entity and relationship type definitions for social simulation
"""
...
- We need to simulate how various parties react and how information propagates during public opinion events
...
A. **Fallback types (must be included, placed as the last 2 in the list)**:
   - `Person`: Fallback type for any individual person. When a person does not belong to other more specific person types, they fall into this category.
   - `Organization`: Fallback type for any organization. When an organization does not belong to other more specific organization types, it falls into this category.
...
- Various people appear in texts, such as "elementary school teachers", "bystanders", "anonymous netizens"
- If there is no specific type match, they should be categorized under `Person`
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        # If text exceeds 50k chars, truncate (only affects what is sent to LLM, not graph building)
...
5. Attribute names cannot use name, uuid, group_id, etc. as reserved words; use full_name, org_name, etc. instead
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Final check to ensure limits (defensive programming)
