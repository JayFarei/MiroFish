...
1. InsightForge (deep insight retrieval) - most powerful hybrid retrieval, auto-generates sub-queries
...
class InsightForgeResult:
    ...
    Contains retrieval results from multiple sub-queries and consolidated analysis
    """
    query: str
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            f"## Panorama Search Results (Future Panoramic View)",
            f"Query: {self.query}",
            f"\n### Statistics",
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter out junk content with question numbers
                skip = False
                for d in '123456789':
                ...
                if skip:
                    continue
                # Truncate long content (by period, not hard cut)
...
class InterviewResult:
    ...
    interview_questions: List[str]  # Interview questions list
...
class ZepToolsService:
    ...
    def search_graph(
        ...
        Uses hybrid search (semantic + BM25) to search the graph.
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        Get all edges from graph (paginated, with temporal info)
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        try:
            ...
            result = []
            for edge in all_edges:
                # Check if edge is related to the node (as source or target)
    ...
    def get_entity_summary(
        ...
        Search for all info related to the entity and generate a summary
    ...
    def get_simulation_context(
        ...
        # Filter entities with actual types (not plain Entity nodes)
        entities = []
        for node in all_nodes:
    ...
    def insight_forge(
        ...
        Most powerful hybrid retrieval, auto-decomposes queries for multi-dimensional search:
        1. Use LLM to decompose query into sub-queries
        2. Semantic search per sub-query
        3. Extract related entities and get their details
        4. Trace relationship chains
        5. Consolidate results into deep insights
        
        Args:
            graph_id: Graph ID
            query: User query
            simulation_requirement: Simulation requirement description
            report_context: Report context (optional, for more precise sub-queries)
        ...
        # Step 3: Extract related entity UUIDs from edges, get only these entities
        entity_uuids = set()
    ...
    def _generate_sub_queries(
        ...
        Decompose complex queries into independently searchable sub-queries
        """
...
Return the sub-question list in JSON format."""
    ...
    def panorama_search(
        ...
        Suitable for understanding the full picture and tracking evolution.
    ...
    def interview_agents(
        ...
        1. Auto-read profile files for all simulated agents
        2. Use LLM to analyze requirements and select most relevant agents
        3. Use LLM to generate interview questions
        4. Call /api/simulation/interview/batch for real interviews (dual platform)
        5. Consolidate results into interview report
        
        Important: Requires simulation environment to be running (OASIS env not closed)
        
        Use cases:
        - Get perspectives from different roles
        - Collect opinions from multiple parties
        - Get real agent responses (not LLM simulated)
        
        Args:
            simulation_id: Simulation ID (for locating profiles and calling interview API)
        ...
        INTERVIEW_PROMPT_PREFIX = (
            ...
            "directly answer the following questions in plain text.\n"
        ...
        # Step 4: Call real interview API (no platform specified, defaults to dual platform)
        try:
            # Build batch interview list (no platform, dual platform)
            interviews_request = []
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                clean_text = re.sub(r'(Question|Question)\s*\d+[：:]\s*', '', clean_text)
                ...
                # Strategy 2 (supplementary): properly paired Chinese quoted text
                if not key_quotes:
            ...
            result.summary = f"Interview failed: {str(e)}. The simulation environment may be closed. Please ensure OASIS is running."
            return result
        except Exception as e:
    ...
    def _clean_tool_call_response(response: str) -> str:
        """Clean JSON tool call wrappers from agent response, extract actual content"""
