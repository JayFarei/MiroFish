...
Reads nodes from Zep graph, filters those matching predefined entity types
"""
...
class ZepEntityReader:
    ...
    def _call_with_retry(
        ...
        """
        Zep API call with retry

        Args:
            func: Function to execute (no-arg lambda or callable)
            operation_name: Operation name for logging
            max_retries: Max retries (default 3, i.e. up to 3 attempts)
    ...
    def get_node_edges(self, node_uuid: str) -> List[Dict[str, Any]]:
        """
        Get all related edges for a given node (with retry)
    ...
    def filter_defined_entities(
        ...
        Filtering logic:
        - If a node's Labels only contain 'Entity', it doesn't match our predefined types, skip
        - If a node's Labels contain labels besides 'Entity' and 'Node', it matches, keep

        Args:
            graph_id: Graph ID
            defined_entity_types: Predefined entity type list (optional, filters to these types only)
        ...
        # Build UUID-to-node-data mapping
        ...
        for node in all_nodes:
            ...
            # Filter: Labels must include labels beyond "Entity" and "Node"
