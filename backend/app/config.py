...
Loads config from the .env file at project root
"""
...
if os.path.exists(project_root_env):
    ...
    # If no .env at root, try loading env vars (for production)
...
class Config:
    ...
    # JSON config - disable ASCII escaping for direct CJK display (instead of \uXXXX)
    JSON_AS_ASCII = False
    
    # LLM backend: "openai" (OpenAI-compatible API) or "claude_code" (Claude Code headless)
    ...
    def validate(cls):
        ...
        errors = []
        if cls.LLM_BACKEND == 'openai' and not cls.LLM_API_KEY:
